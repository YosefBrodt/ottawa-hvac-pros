...
type StarsProps = {
  rating: number;
  size?: 'sm' | 'md';
  /** light: gold on light cards · dark: on navy · amber: Stitch review strip */
  variant?: 'light' | 'dark' | 'amber';
};

export function Stars({ rating, size = 'md', variant = 'light' }: StarsProps) {
  const dim =
    variant === 'dark' ? 'rgba(255,255,255,0.12)' : variant === 'amber' ? 'rgba(0,0,0,0.12)' : '#e8ecf3';
  const className = size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4';
  const activeFill = variant === 'amber' ? AMBER : GOLD;
